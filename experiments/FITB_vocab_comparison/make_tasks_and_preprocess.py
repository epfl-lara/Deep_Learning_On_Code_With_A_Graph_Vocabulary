...
if __name__ == '__main__':
    kwargs = dict(seed=515,
                  ...
                  model_names_labels_and_prepro_kwargs=[
                      ('FITBFixedVocabGGNN', 'all_edge', frozenset(),
                       dict(),
                       dict(max_nodes_per_graph=500)),
                      ('FITBCharCNNGGNN', 'all_edge', frozenset(),
                       dict(max_name_encoding_length=30),
                       dict(max_nodes_per_graph=500)),
                      ('FITBNameGraphVocabGGNN', 'all_edge', frozenset(),
                       dict(max_name_encoding_length=30),
                       dict(max_nodes_per_graph=500)),
                      ('FITBFixedVocabGGNN', 'syntax_edge', syntax_only_excluded_edge_types,
                       dict(),
                       dict(max_nodes_per_graph=500)),
                      ('FITBCharCNNGGNN', 'syntax_edge', syntax_only_excluded_edge_types,
                       dict(max_name_encoding_length=30),
                       dict(max_nodes_per_graph=500)),
                      ('FITBNameGraphVocabGGNN', 'syntax_edge', syntax_only_excluded_edge_types,
                       dict(max_name_encoding_length=30),
                       dict(max_nodes_per_graph=500)),
                  ],
