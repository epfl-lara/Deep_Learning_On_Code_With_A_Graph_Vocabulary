...
if __name__ == '__main__':
    kwargs = dict(seed=515,
                  ...
                  model_names_labels_and_prepro_kwargs=[
                      ('VarNamingNameGraphVocabGGNN', 'all_edge_except_subtoken', frozenset(),
                       dict(max_name_encoding_length=30,
                            add_edges=False),
                       dict(max_nodes_per_graph=500)),
                      ('VarNamingNameGraphVocabGGNN', 'syntax_edge_except_subtoken', syntax_only_excluded_edge_types,
                       dict(max_name_encoding_length=30,
                            add_edges=False),
                       dict(max_nodes_per_graph=500)),
                  ],
                  skip_make_tasks=False)
    run_command_on_remote(aws_config['remote_ids']['preprobox'],
