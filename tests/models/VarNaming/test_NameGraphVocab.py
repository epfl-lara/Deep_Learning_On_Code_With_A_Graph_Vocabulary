...
from models import VarNamingNameGraphVocabGGNN
from models.VarNaming.NameGraphVocab import VarNamingNameGraphVocab, VarNamingNameGraphVocabDataEncoder, \
    VarNamingNameGraphVocabDataPoint
...
class TestVarNamingNameGraphVocabDataEncoder(unittest.TestCase):
    def setUp(self):
        self.gml_dir = os.path.join(test_s3shared_path, 'test_dataset', 'repositories')
        self.output_dataset_dir = os.path.join(test_s3shared_path, 'VarNaming_NameGraphVocab_dataset')
        self.test_gml_files = []
    ...
    def test_init_finds_all_relevant_dataset_information(self):
        de = VarNamingNameGraphVocabDataEncoder(self.task.graphs_and_instances,
    ...
    def test_encode(self):
        ...
        for graph, instances in self.task.graphs_and_instances:
            VarNamingNameGraphVocab.fix_up_edges(graph, instances, frozenset())
            VarNamingNameGraphVocab.extra_graph_processing(graph, instances, de)
            for instance in tqdm(instances):
                dporig = VarNamingNameGraphVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=50)
                dp = deepcopy(dporig)
                de.encode(dp)
...
class TestVarNamingNameGraphVocab(unittest.TestCase):
    def setUp(self):
        self.gml_dir = os.path.join(test_s3shared_path, 'test_dataset', 'repositories')
        self.output_dataset_dir = os.path.join(test_s3shared_path, 'VarNaming_NameGraphVocab_dataset')
    ...
    def test_preprocess_task_type_check_basic_functionality(self):
        ...
        with self.assertRaises(AssertionError):
            VarNamingNameGraphVocab.preprocess_task(task)

    def test_preprocess_task_existing_encoding_basic_functionality(self):
        VarNamingNameGraphVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30,
                                                data_encoder='new',
                                                data_encoder_kwargs=dict(
                                                    max_name_encoding_length=self.max_name_encoding_length),
                                                instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        de = VarNamingNameGraphVocabDataEncoder.load(
            os.path.join(self.output_dataset_dir, '{}.pkl'.format(VarNamingNameGraphVocabDataEncoder.__name__)))
        VarNamingNameGraphVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30,
    ...
    def test_instance_to_datapoint(self):
        for excluded_edge_types in [syntax_only_excluded_edge_types, frozenset()]:
            ...
            for graph, instances in tqdm(self.task.graphs_and_instances):
                VarNamingNameGraphVocab.fix_up_edges(graph, instances, excluded_edge_types)
                VarNamingNameGraphVocab.extra_graph_processing(graph, instances, de)
                node_names = []
                ...
                for instance in instances:
                    dp = VarNamingNameGraphVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=100)
                    self.assertEqual(type(dp), VarNamingNameGraphVocabDataPoint)
    ...
    def test_batchify_and_unbatchify_are_inverses(self):
        VarNamingNameGraphVocab.preprocess_task(self.task,
                                                output_dir=self.output_dataset_dir,
                                                n_jobs=30,
                                                data_encoder='new',
                                                data_encoder_kwargs=dict(
                                                    max_name_encoding_length=self.max_name_encoding_length),
                                                instance_to_datapoints_kwargs=dict(max_nodes_per_graph=100))
        with open(os.path.join(self.output_dataset_dir, '{}.pkl'.format(VarNamingNameGraphVocab.DataEncoder.__name__)),
                  'rb') as f:
            de = pickle.load(f)
        model = VarNamingNameGraphVocabGGNN(data_encoder=de,
                                            hidden_size=17,
                                            type_emb_size=5,
                                            name_emb_size=7,
                                            n_msg_pass_iters=1,
        ...
        for b in tqdm(range(int(math.ceil(len(datapoints) / batch_size)))):
            ...
            self.assertAlmostEqual((model_output.sum(axis=2) - 1).sum().asscalar(), 0, 4,
