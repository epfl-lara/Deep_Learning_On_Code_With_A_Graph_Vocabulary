...
from models import FITBNameGraphVocabGGNN
from models.FITB.FITBModel import too_useful_edge_types
from models.FITB.NameGraphVocab import FITBNameGraphVocab, FITBNameGraphVocabDataEncoder, FITBNameGraphVocabDataPoint
...
class TestFITBNameGraphVocabDataEncoder(unittest.TestCase):
    def setUp(self):
    ...
    def test_init_finds_all_relevant_dataset_information(self):
        de = FITBNameGraphVocabDataEncoder(self.task.graphs_and_instances,
    ...
    def test_encode(self):
        ...
        for graph, instances in self.task.graphs_and_instances:
            FITBNameGraphVocab.fix_up_edges(graph, instances, frozenset())
            FITBNameGraphVocab.extra_graph_processing(graph, instances, de)
            for instance in tqdm(instances):
                dporig = FITBNameGraphVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=50)
                dp = deepcopy(dporig)
                de.encode(dp)
...
class TestFITBNameGraphVocab(unittest.TestCase):
    def setUp(self):
        self.gml_dir = os.path.join(test_s3shared_path, 'test_dataset', 'repositories')
        self.output_dataset_dir = os.path.join(test_s3shared_path, 'FITB_NameGraphVocab_dataset')
    ...
    def test_preprocess_task_type_check_basic_functionality(self):
        ...
        with self.assertRaises(AssertionError):
            FITBNameGraphVocab.preprocess_task(task)

    def test_preprocess_task_existing_encoding_basic_functionality(self):
        FITBNameGraphVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30, data_encoder='new',
                                           data_encoder_kwargs=dict(
                                               max_name_encoding_length=self.max_name_encoding_length),
                                           instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        de = FITBNameGraphVocabDataEncoder.load(
            os.path.join(self.output_dataset_dir, '{}.pkl'.format(FITBNameGraphVocabDataEncoder.__name__)))
        FITBNameGraphVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30, data_encoder=de)
        with self.assertRaises(AssertionError):
            de = BaseDataEncoder(dict(), frozenset())
            FITBNameGraphVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30,
                                               data_encoder=de)
    ...
    def test_preprocess_task_for_model(self):
        ...
        FITBNameGraphVocab.preprocess_task(task=task,
        ...
        data_encoder = FITBNameGraphVocab.DataEncoder.load(os.path.join(self.output_dataset_dir,
                                                                        'FITBNameGraphVocabDataEncoder.pkl'))
        ...
        for file in os.listdir(self.output_dataset_dir):
            if file not in ['FITBNameGraphVocabDataEncoder.pkl', 'FITBTask.pkl', 're-encoding']:
    ...
    def test_instance_to_datapoint(self):
        for excluded_edge_types in [syntax_only_excluded_edge_types, frozenset()]:
            de = FITBNameGraphVocab.DataEncoder(self.task.graphs_and_instances,
                                                excluded_edge_types=excluded_edge_types,
                                                instance_to_datapoints_kwargs=dict(),
                                                max_name_encoding_length=self.max_name_encoding_length)
            for graph, instances in tqdm(self.task.graphs_and_instances):
                FITBNameGraphVocab.fix_up_edges(graph, instances, excluded_edge_types)
                FITBNameGraphVocab.extra_graph_processing(graph, instances, de)
                node_names = []
                ...
                for instance in instances:
                    dp = FITBNameGraphVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=100)
                    self.assertEqual(type(dp), FITBNameGraphVocabDataPoint)
    ...
    def test_batchify_and_unbatchify_are_inverses(self):
        FITBNameGraphVocab.preprocess_task(self.task,
                                           output_dir=self.output_dataset_dir,
                                           n_jobs=30,
                                           data_encoder='new',
                                           data_encoder_kwargs=dict(
                                               max_name_encoding_length=self.max_name_encoding_length),
                                           instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        with open(os.path.join(self.output_dataset_dir, '{}.pkl'.format(FITBNameGraphVocab.DataEncoder.__name__)),
                  'rb') as f:
            de = pickle.load(f)
        model = FITBNameGraphVocabGGNN(data_encoder=de,
                                       hidden_size=17,
                                       type_emb_size=5,
                                       name_emb_size=7,
                                       n_msg_pass_iters=1)
