...
from models import FITBFixedVocabGGNN
from models.FITB.FITBModel import too_useful_edge_types
from models.FITB.FixedVocab import FITBFixedVocabDataEncoder, FITBFixedVocab, FITBFixedVocabDataPoint
...
class TestFixedVocabDataEncoder(unittest.TestCase):
    def setUp(self):
        ...
        self.output_dataset_dir = os.path.join(test_s3shared_path, 'FITB_Fixed_Vocab_dataset')
        self.test_gml_files = []
    ...
    def test_init_finds_all_relevant_dataset_information(self):
        de = FITBFixedVocabDataEncoder(self.task.graphs_and_instances, excluded_edge_types=frozenset(),
    ...
    def test_encode(self):
        ...
        for graph, instances in self.task.graphs_and_instances:
            for instance in tqdm(instances):
                dporig = FITBFixedVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=50)
                dp = deepcopy(dporig)
                de.encode(dp)
...
class TestFITBFixedVocab(unittest.TestCase):
    def setUp(self):
        self.gml_dir = os.path.join(test_s3shared_path, 'test_dataset', 'repositories')
        self.output_dataset_dir = os.path.join(test_s3shared_path, 'FITB_Fixed_Vocab_dataset')
    ...
    def test_preprocess_task_type_check_basic_functionality(self):
        ...
        with self.assertRaises(AssertionError):
            FITBFixedVocab.preprocess_task(task)

    def test_preprocess_task_existing_encoding_basic_functionality(self):
        FITBFixedVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30, data_encoder='new',
                                       data_encoder_kwargs=dict(),
                                       instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        de = FITBFixedVocabDataEncoder.load(
            os.path.join(self.output_dataset_dir, '{}.pkl'.format(FITBFixedVocabDataEncoder.__name__)))
        FITBFixedVocab.preprocess_task(self.task, output_dir=self.output_dataset_dir, n_jobs=30, data_encoder=de,
                                       instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        with self.assertRaises(AssertionError):
    ...
    def test_instance_to_datapoint(self):
        for excluded_edge_types in [syntax_only_excluded_edge_types, frozenset()]:
            de = FITBFixedVocab.DataEncoder(self.task.graphs_and_instances, excluded_edge_types=excluded_edge_types,
                                            instance_to_datapoints_kwargs=dict())
            for graph, instances in tqdm(self.task.graphs_and_instances):
                FITBFixedVocab.fix_up_edges(graph, instances, excluded_edge_types)
                FITBFixedVocab.extra_graph_processing(graph, instances, de)
                for instance in instances:
                    dp = FITBFixedVocab.instance_to_datapoint(graph, instance, de, max_nodes_per_graph=100)
                    self.assertEqual(type(dp), FITBFixedVocabDataPoint)
    ...
    def test_preprocess_task_for_model(self):
        ...
        FITBFixedVocab.preprocess_task(task=task,
        ...
        data_encoder = FITBFixedVocab.DataEncoder.load(os.path.join(self.output_dataset_dir,
        ...
        for file in os.listdir(self.output_dataset_dir):
            if file not in ['FITBFixedVocabDataEncoder.pkl', 'FITBTask.pkl', 're-encoding']:
    ...
    def test_batchify_and_unbatchify_are_inverses(self):
        FITBFixedVocab.preprocess_task(self.task,
                                       output_dir=self.output_dataset_dir,
                                       n_jobs=30,
                                       data_encoder='new',
                                       data_encoder_kwargs=dict(),
                                       instance_to_datapoints_kwargs=dict(max_nodes_per_graph=20))
        with open(os.path.join(self.output_dataset_dir, '{}.pkl'.format(FITBFixedVocab.DataEncoder.__name__)),
                  'rb') as f:
            de = pickle.load(f)
        model = FITBFixedVocabGGNN(data_encoder=de,
                                   hidden_size=17,
                                   type_emb_size=5,
                                   name_emb_size=7,
                                   n_msg_pass_iters=1)
