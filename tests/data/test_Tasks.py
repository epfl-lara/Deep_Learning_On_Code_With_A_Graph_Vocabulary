...
class TestFITBTask(TestTask):
    ...
    def test_all_variables_included(self):
        for pg, var_uses in self.task.graphs_and_instances:
            for node, _ in pg.nodes_that_represent_variables:
                if len(pg.get_all_variable_usages(node)) > 1:
    ...
    def test_other_uses_are_correct(self):
        for pg, var_uses in self.task.graphs_and_instances:
            for var_use, other_uses in var_uses:
                ...
                self.assertCountEqual(pg.get_all_variable_usages(var_use), other_uses + (var_use,))
...
class TestVarNamingTask(TestTask):
    ...
    def test_all_variables_included(self):
        for pg, var_names_locs in self.task.graphs_and_instances:
            locations = list(chain.from_iterable([i[1] for i in var_names_locs]))
            self.assertCountEqual(locations, [i[0] for i in pg.nodes_that_represent_variables])
