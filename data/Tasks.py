...
class Task:
    ...
    def from_gml_files(cls, gml_files: List[str]):
        ...
        for gml_file in tqdm(gml_files):
            pg = AugmentedAST.from_gml(gml_file, task.parent_types_of_variable_nodes)
            task.add_AugmentedAST(pg)
        return task
...
class FITBTask(Task):
    ...
    def add_AugmentedAST(self, pg: AugmentedAST) -> None:
        instances = []
        visited_nodes = set()
        for node, data in pg.nodes_that_represent_variables:
            if node not in visited_nodes:
                location_list = pg.get_all_variable_usages(node)
        ...
        self.graphs_and_instances.append((pg, tuple(instances)))
    ...
    def add_AugmentedAST(self, pg: AugmentedAST) -> None:
        ...
        for node, data in pg.nodes_that_represent_variables:
            if node not in included_nodes:
                ...
                locations = pg.get_all_variable_usages(node)
                for loc in locations:
                    assert pg[loc]['identifier'] == real_var_name
