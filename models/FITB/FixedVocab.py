...
class FITBFixedVocabDataEncoder(BaseDataEncoder):
    DataPoint = FITBFixedVocabDataPoint
    ...
    def encode(self, dp: FITBFixedVocabDataPoint) -> None:
        '''
...
class FITBFixedVocab(FITBModel):
    '''
    Model that relies on a fixed vocabulary to do the FITB task
    '''

    DataEncoder = FITBFixedVocabDataEncoder
    InputClass = FixedVocabInput

    @staticmethod
    def instance_to_datapoint(graph: AugmentedAST,
                              instance,
                              data_encoder: FITBFixedVocabDataEncoder,
